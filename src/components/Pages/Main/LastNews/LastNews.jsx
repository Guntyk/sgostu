  ...
  return (
    articles.length > 0 && (
      <article className='last-news news-section'>
        <span className='last-news-title'>{language === 'en' ? 'Latest news' : 'Останні новини'}</span>
          ...
          <div className='last-news-wrapper'>
            {articles.length !== 0 ? (
              articles
                ...
                .slice(-3)
                .reverse()
                .map((article) => <LastNewsCard key={article.id} article={article.attributes} articleId={article.id} />)
            ) : (
              <span className='event-void'>{language === 'en' ? 'No news' : 'Новин немає'}</span>
            )}
          </div>
        </div>
        <Swiper

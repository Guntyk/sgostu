export const socialsFormatting = (socialType, socialField) => {
  if (socialType === 'phone') {
    ...
    !letters.map((letter) => socialField.toLowerCase().includes(letter)).includes(true) &&
    ...
    return socialType === 'tiktok' ? `https://tiktok.com/@${socialField}` : `https://${socialType}.com/${socialField}`;
  } else {
    return;
  }
};
